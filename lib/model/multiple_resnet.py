...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, target_dataset="cifar10"):
        super(ResNet, self).__init__()
        self.in_planes = 64
        # CIFAR-10, input size=(3, 32, 32)
        if (
            target_dataset == "cifar10"
            or target_dataset == "svhn"
            or target_dataset == "tiny_imagenet"
        ):
        ...
        # ms1m, input size=(3, 112, 112)
        elif target_dataset in {"ms1m", "celeba", "dogs100", "non-dogs"}:
            self.conv1 = nn.Conv2d(
                3, self.in_planes, kernel_size=7, stride=2, padding=3, bias=False
            )
            self.max_pool = lambda x: x
            ...
            or target_dataset == "fmnist"
            or target_dataset == "emnist"
        ):
            self.conv1 = nn.Conv2d(
...
def ResNet34(num_c, target_dataset):
    return ResNet(
        BasicBlock, [3, 4, 6, 3], num_classes=num_c, target_dataset=target_dataset
    )
